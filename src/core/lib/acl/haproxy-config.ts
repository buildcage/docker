 ...
 *  2. The requested name is resolved here and connected to (`do-resolve` then
 *     `set-dst`), so a forged Host or doctored /etc/hosts cannot pick the
 *     destination.
...
export interface HaproxyConfigOptions extends RuleInputs {
  ...
  listenPort?: number;
  /** The resolver the build uses. HAProxy has to use the same one. */
...
export function generateHaproxyConfig(options: HaproxyConfigOptions = {}): GeneratedHaproxyConfig {
  ...
  if (opts.resolverAddress) {
    l.push(
      "# The resolver names are looked up through, the same one the build uses.",
      "resolvers buildcage",
      `    nameserver ns1 ${opts.resolverAddress}:53`,
      "    resolve_retries 3",
      "    timeout resolve 3s",
      "    timeout retry 1s",
      "    hold valid 30s",
  ...
  if (ipRules.length > 0 || tlsHosts.length > 0) {
    ...
    if (tlsHosts.length > 0 && opts.resolverAddress) {
      ...
      l.push(
        ...
        `    acl pass_dst_internal var(txn.dst) -m ip ${INTERNAL_RANGES.join(" ")} ${opts.resolverAddress}`,
  ...
  const stage = (
    ...
    if (opts.resolverAddress) {
      l.push(
        ...
        "    # rules below still decide, so nothing is loosened.",
        ...
        `    acl dst_internal var(txn.dst) -m ip ${INTERNAL_RANGES.join(" ")} ${opts.resolverAddress}`,
