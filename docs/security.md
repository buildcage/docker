...
- The image (`docker/explicit/Dockerfile`) has no s6-overlay, no HAProxy, no dnsmasq, and no
  iptables rules. A small statically-linked Go binary (`docker/explicit/buildkit-proxy/`) is the
  container's entrypoint (PID 1) and directly supervises the real `buildkitd` as a child process.
...
- **`RUN --network=host`** bypasses the CNI network entirely for the proxy's own egress (it dials
  out through the daemon's own host network namespace instead), in addition to requiring the usual
