...
- **`buildcage-runc`** wraps BuildKit's own `buildkit-runc`. For the subcommands that carry a
  ...
  files, all of which are taken back out once the step has exited. A step that starts without a store and
  installs `ca-certificates` partway through is the one case that keeps a trace: the store written
  for it is in `/etc/ssl/certs` when the package's own `update-ca-certificates` runs, so that step's
  log carries a line about it, and counts the injected certificate among the ones it added. The
  store itself, and everything the rebuild left beside it, still come back out. Injection happens at exec time, never touches LLB, and so cannot
  affect a cache key.
