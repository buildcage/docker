...
# to invoke the in-container report mechanism.
#
...
#   into the {{...}} placeholders core/scripts/report.js's step summary
#   leaves behind — those two values only exist in the report action step's
#   own runtime, not inside the container.
#
...
raw_log=$(docker exec "$container_id" sh -c 'cat /var/log/haproxy/current 2>/dev/null || true')
