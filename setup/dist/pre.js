...
async function downloadFile(url, destPath) {
  ...
  node_fs.writeFileSync(destPath, Buffer.from(await resp.arrayBuffer()), { mode: 0o755 });
}
...
async function installCosign() {
  ...
  const installDir = node_path.join(node_os.homedir(), ".cosign");
  node_fs.mkdirSync(installDir, { recursive: true });
  const binPath = node_path.join(installDir, "cosign");
  ...
  node_fs.appendFileSync(process.env.GITHUB_PATH, `${installDir}\n`);
  return binPath;
}
...
function getManifestDigest(imageRef) {
  try {
    const result = node_child_process.execFileSync(
      "docker",
...
function verifyImage(cosignPath, image, digest, actionRef, actionRepo) {
  ...
  try {
    node_child_process.execFileSync(cosignPath, args, { stdio: "inherit" });
...
async function main() {
  ...
  node_fs.appendFileSync(process.env.GITHUB_STATE, `BUILDCAGE_DIGEST=${digest}\n`);
